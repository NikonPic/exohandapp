...
import '../03_database/user.dart';
import '../03_database/user_database.dart';
...
class Exoskeleton {
  ...
  void update(List<int> intMessage) {
    // assign message
    ms = intMessage[0];
    angleB = (timeFilter * angleB + (1 - timeFilter) * intMessage[1]).toInt();
    angleA = (timeFilter * angleA + (1 - timeFilter) * intMessage[2]).toInt();
    angleK = (timeFilter * angleK + (1 - timeFilter) * intMessage[3]).toInt();
    forceB = intMessage[4];
    forceA = intMessage[5];
    // calibrate
    calibrate();
  ...
  void addRawArrs() {
    // raw data
    timeArr.add(ms);
    degBarrRaw.add(angleB);
    degAarrRaw.add(angleA);
    degKarrRaw.add(angleK);
    forceBarr.add(forceB);
    forceAarr.add(forceA);
  }
...
class ExoskeletonAdv extends Exoskeleton {
  ...
  void getActuationForce() {
    final double alphaQuer = pi - theta - phi1;
  ...
  void getJointTorques() {
    ...
    // add results
    mMcpNmArr.add(mMcpNmm / 1000);
    mPipNmArr.add(mPipNmm / 1000);
    mDipNmArr.add(mDipNmm / 1000);
  }
}
